...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"number": "J013S", "provisional_designation": ""},
            "Natural Satellite",
            "Jupiter XIII",
        ),
...
def test_identify_object_missing_ids():
    ...
    with pytest.raises(
        ValueError, match="Missing both permanent ID and provisional ID"
    ):
        identify_object(row)
...
def test_batch_mpc_vs_parser_consistency():
    ...
    parsed_ids = (
        table_main["number"].astype(str).tolist()
        + table_sat["number"].astype(str).tolist()
    )
