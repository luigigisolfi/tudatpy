...
def test_nrlmsise00():
    ...
    with open(
        get_atmosphere_tables_path() + "/nrlmsise00_validation_data.pkl", "rb"
    ) as file:
        validation_data = pickle.load(file)

    # Initialise Tudat NRLMSISE00 model
    solar_weather_data = read_solar_activity_data(
    ...
    computed_densities = np.array(
        [
            ...
            for alt, lon, lat, epoch in zip(
                altitudes, longitudes, geodetic_latitudes, epochs
            )
        ]
    )
    ...
    relative_differences = (
        np.abs(computed_densities - expected_densities) / expected_densities
    )
