...
def unpack_permanent_minor_planet(packed: str) -> str:
    ...
    if len(packed) != 5:
        raise ValueError(
            f"Invalid packed length: '{packed}'. Must be exactly 5 characters."
        )
    ...
    # Format: 5 Digits
        ...
        if not packed.isdigit():
            raise ValueError(
                f"Invalid format for basic range: '{packed}'. Must be all digits."
            )
...
def unpack_provisional_minor_planet(packed: str) -> str:
    ...
    if len(packed) != 7:
        raise ValueError(
            f"Invalid packed length: '{packed}'. Must be exactly 7 characters."
        )
