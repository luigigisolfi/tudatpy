...
def parse_80cols_data(lines: list[str]) -> Table:
    ...
    human_readable_number = ident_info["unpacked_number"].fillna(
        ident_info["unpacked_name"]
    )
    ...
    ra_deg = (
        df_obs["ra_h_n"] + df_obs["ra_m_n"] / 60.0 + df_obs["ra_s_n"] / 3600.0
    ) * 15.0
...
def identify_object(row: pd.Series) -> pd.Series:
    ...
    if perm_id:
        if re.match(r"^[JSUND]\d{3}S$", perm_id):
            ...
            if perm_id[0] in unpackers.PLANET_MAP:
                result["unpacked_name"] = unpackers.unpack_permanent_natural_satellite(
                    perm_id
                )
    ...
    # --- PATH B: ONLY PROVISIONAL ID IS PRESENT ---
        ...
        if len(prov_id) == 7 and prov_id[6].isalpha() and prov_id[6] not in ["I", "Z"]:
            ...
            result["unpacked_name"] = unpackers.unpack_provisional_comet_or_satellite(
                prov_id
            )
...
def parse_packed_permanent_designation(packed_perm_num: str) -> dict[str, str]:
    ...
    if re.match(r"^[JSUND]\d{3}S$", packed_perm_num):
        ...
        ident_data["name"] = unpackers.unpack_permanent_natural_satellite(
            packed_perm_num
        )
    ...
    if packed_perm_num:
        ...
        if packed_prov_desig:
            if ident_data.get("type") == "Minor Planet":
                ident_data["desig"] = unpackers.unpack_provisional_minor_planet(
                    packed_prov_desig
                )
        ...
        if packed_prov_desig[6].isalpha() and packed_prov_desig[6] not in ["I", "Z"]:
            ...
            ident_data["type"] = "Comet"
            ident_data["desig"] = unpackers.unpack_provisional_comet_or_satellite(
                packed_prov_desig
            )
